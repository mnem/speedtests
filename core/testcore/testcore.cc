...
TestRun* NewTestRun(
  ...
  test_run = (TestRun *)malloc(sizeof(TestRun));
  assert(test_run);
  ...
  test_run->run_durations = (clock_t *)malloc(sizeof(clock_t) * runs);
  assert(test_run->run_durations);
  memset(test_run->run_durations, 0xff, sizeof(int) * runs);

  test_run->description = (char *)malloc(strlen(description) + 1);
...
void RunTest(TestRun* test_run,
  ...
  for (i = 0; i < test_run->runs; i++) {
    ...
    before = clock();
    test_run->Test(test_run->user_data);
